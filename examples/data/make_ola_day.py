...
Runs only where the source archive is mounted; it is not needed to use the notebook.

Source (not public):
  ec_cache/ola_001_irt.h5      1 Hz FW / T_CANOPY, keyed by block index
...
The IRT cache is keyed by BLOCK INDEX, not by timestamp: dataset "1234" of each channel
group is the block starting at meta/timestamp[1234] in the sonic cache. Reading it over a
network volume is slow (thousands of small random reads); copy it locally for more than a
day.

Writes:
  ola_2023-05-11_1hz.csv.gz    TIMESTAMP, FWT, IRT           (86,400 rows)
